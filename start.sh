...
then
  ...
  java -Dlog4j.configurationFile=file:./log4j2.xml -Dlog.path=${log_path} -Dmodule.name=${module_name} -Dlog.level=${log_level} -Dspring.config.location=file:./application.properties,file:${commonConfigurationFile} -jar ${build} 1>/dev/null 2>${log_path}/${module_name}.error &
